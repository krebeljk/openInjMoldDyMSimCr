...
Foam::taitCr<Specie>::taitCr(Istream& is)
:
    Specie(is),
    b1m_(readScalar(is)),
    b2m_(readScalar(is)),
    ...
    b10_(readScalar(is))
{
    is.check("taitCr<Specie>::taitCr(Istream& is)");
}


template<class Specie>
Foam::taitCr<Specie>::taitCr(const dictionary& dict)
:
    Specie(dict),
...
// * * * * * * * * * * * * * * * Podatki * * * * * * * * * * * * * //
template<class Specie>
Foam::scalar Foam::taitCr<Specie>::C_ = 0.0894; // to se pozneje posplosi

// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

template<class Specie>
void Foam::taitCr<Specie>::write(Ostream& os) const
{
    Specie::write(os);
...
template<class Specie>
Foam::Ostream& Foam::operator<<(Ostream& os, const taitCr<Specie>& pf)
{
    ...
    os.check("Ostream& operator<<(Ostream&, const taitCr<Specie>&)");
    return os;
}
